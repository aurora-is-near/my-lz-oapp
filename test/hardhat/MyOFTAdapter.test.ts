...
describe('MyOFTAdapter Test', function () {
    ...
    before(async function () {
        ...
        MyERC20 = await ethers.getContractFactory('MyERC20Mock')
    ...
    beforeEach(async function () {
        ...
        myERC20 = await MyERC20.deploy('TEST', 'TEST')

        // Deploying two instances of MyOFT contract with different identifiers and linking them to the mock LZEndpoint
        myOFTA = await MyOFTAdapter.deploy(myERC20.address, mockEndpointV2A.address, ownerA.address)
    ...
    it('should send a ERC20 token from A address to B address via OFTAdapter', async function () {
        // Minting an initial amount of tokens to ownerA's address in the myOFTA contract
        const initialAmount = ethers.utils.parseEther('100')
        await myERC20.mint(ownerA.address, initialAmount)
        ...
        const finalBalanceA = await myERC20.balanceOf(ownerA.address)
